        ...
        <div className="m-8">
            ...
            <div className="flex gap-4 overflow-auto ">
                {catalogue.filter(item => item.type === type).map((item, i) => (
                    <div key={i} className="mb-4 min-h-[300px] h-[380px] max-w-[300px] aspect-video border-2 rounded-3xl shadow-md">
                        <h2 className="text-center font-bold text-[1.2em]">{item.name}</h2>
                        <Link to={`/product/${item.id}`}>
                            <img src={item.images[0]} alt={item.name} className="w-full h-[200px] object-contain mb-2 rounded-2xl" />
                        </Link>
                        <hr />
